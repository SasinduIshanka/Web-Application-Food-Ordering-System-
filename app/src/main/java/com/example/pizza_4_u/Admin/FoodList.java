...
public class FoodList extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        gridView = (GridView) findViewById(R.id.gridViewF);
        list = new ArrayList<>();
        adapter = new FoodListAdapter(this, R.layout.food_items, list);
        gridView.setAdapter(adapter);

        // get all data from sqlite
        Cursor cursor = Admin_home.sqLiteHelper.getData("SELECT * FROM FOOD");
        list.clear();
            ...
            public boolean onItemLongClick(AdapterView<?> parent, View view, final int position, long id) {
                ...
                dialog.setItems(items, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int item) {
                        if (item == 0) {
                            // update
                            Cursor c = Admin_home.sqLiteHelper.getData("SELECT id FROM FOOD");
    ...
    private void showDialogUpdate(Activity activity, final int position){
        ...
        btnUpdate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    Admin_home.sqLiteHelper.updateData(
                            edtName.getText().toString().trim(),
                            edtPrice.getText().toString().trim(),
                            Admin_home.imageViewToByte(imageViewFood),
                            position
                    );
                    dialog.dismiss();
    ...
    private void showDialogDelete(final int idFood){
        ...
        dialogDelete.setPositiveButton("OK", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                try {
                    Admin_home.sqLiteHelper.deleteData(idFood);
